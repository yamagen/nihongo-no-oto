...
# 1. **Attention drift（注意の漂流） ⇔ 注意の拡散・注意の逸脱**
...
# 2. **Pattern override（パターン優勢） ⇔ スキーマ／ヒューリスティックの自動発火**

### AI 側

- 文脈より“よく似た過去のパターン”が優先されて誤答につながる。
...
**対応:**
AI の間違いは、人間の **ヒューリスティック（直感パターン）** の暴走に近い。

---

# 3. **Context collapse（文脈崩壊） ⇔ ワーキングメモリ負荷による理解の破綻**
...
**対応:**
AI の文脈崩壊は、人間の **認知負荷過多での混乱** と同型。

---

# 4. \*\*Local coherence / global incoherence

（局所的には筋が通るのに、全体としてズレる）
⇔ 局所推論の暴走\*\*
...
**対応:**
AI の典型的誤答は、人間の **局所一致・全体不一致** の現象と一致。

---

# 5. \*\*Activation competition（内部表現の競合）
...
**対応:**
AI の語生成は、人間の **連想記憶の競合** ときわめて近い。

---

# 6. \*\*Implicit goal misalignment（隠れた目的の取り違え）

⇔ 会話の意図読みに失敗する\*\*

### AI 側

- 「あなたが何を最終目的としているか」の推定を誤ると、
  まったく違う方向に“整然と”進む。
...
**対応:**
AI の意図理解のズレは、人間の **意図解釈ミス** に相当。

---

# 7. \*\*Mode switching（推論モードの切り替え）
...
**対応:**
AI も「似たスイッチング」を内部で起こしている。

---

# 8. \*\*Hallucination（根拠のない創作）
...
**対応:**
AI の hallucination は、人間の **認知的補完** に近い。
...
この対応表をもとに **PGM（即時文法／調整文法）でのモデル化**もできます。

- AI の System1 的生成 → 即時文法（IG）
- AI の System2 的整合化 → 調整文法（AG）
- IG/AG の競合モデルとしてのエラー生成
